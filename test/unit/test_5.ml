...
let pass_with_condition g v =
  (* by the contract in f, g must be bigger_10 -> bigger_20 *)
  let vv = g v in
  vv > 100

let [@contract : (bigger_10 -> bigger_20) -> pass_with_condition dep]
...
let test_blame_g_pre () =
  ...
  let g x = 9 in
  must_blame "Dune__exe__Test_5.test_blame_g_pre.(fun)" (fun () -> ignore (f g))
...
let test_blame_g_post () =
  ...
  let g x = 19 in
  must_blame "Dune__exe__Test_5.test_blame_g_post.(fun)" (fun () -> ignore (f g))

let test_blame_dependent () =
  (* blame the client using f *)
  (* it should not pass the condition of dependent contract *)
  let g x = 99 in
  must_blame "Dune__exe__Test_5.test_blame_dependent.(fun)" (fun () -> ignore (f g))

let test_pass () =
  let g x = 101 in
  Alcotest.(check int) "Equal" 101 (f g)
...
let () =
  ...
  run "Function" [
    ...
    "blame", [
      ...
      test_case "depend" `Quick test_blame_dependent;
    ];
  ]
